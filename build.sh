...
perl -pe '
  BEGIN { use MIME::Base64;
    ...
    for my $g (glob("assets/img/gallery/*.jpg"), glob("assets/img/hero-*.jpg"), glob("assets/img/infirmiere-*.jpg"), glob("assets/img/portrait-*.jpg"), glob("assets/img/studio-*.jpg"), glob("assets/img/fond-*.jpg"), glob("assets/img/carte-*.jpg")) { $map{$g} = inl($g, "image/jpeg"); }
  }
